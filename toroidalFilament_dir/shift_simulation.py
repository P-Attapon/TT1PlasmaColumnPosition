...
def simulate(n):
    for _ in tqdm(range(n)):
        ...
        def helper(probe_array, est_horizontal, est_vertical, order=2):
            signal = cal_signal(R_shift, Z_shift, probe_array)
        ...
        #cross 0
        R0, Z0, R0e,Z0e = helper(probe_array=(cross[0][0], cross[2][0], cross[0][2], cross[2][2]),est_horizontal=R0_cal[-1],est_vertical=Z0_cal[-1])
        R0_cal.append(R0)
        R0_err.append(R0e)
        Z0_cal.append(Z0)
        Z0_err.append(Z0e)
        ...
        #cross 1
        R1, Z1, R1e,Z1e = helper([cross[0][0], cross[1][3], cross[0][2],cross[1][2]], R1_cal[-1], Z1_cal[-1])
        R1_cal.append(R1)
        R1_err.append(R1e)
        Z1_cal.append(Z1)
        Z1_err.append(Z1e)
        ...
        #cross 2
        R2, Z2,R2e,Z2e = helper([cross[1][3], cross[2][0], cross[1][2],cross[2][2]], R2_cal[-1],Z2_cal[-1])
        R2_cal.append(R2)
        R2_err.append(R2e)
        Z2_cal.append(Z2)
        Z2_err.append(Z2e)

simulate(num_iteration)

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(5, 5))

iteration = list(range(num_iteration+1))
...
mean_R0_err = np.mean(abs(np.array(R0_cal) - R_sim))
mean_R1_err = np.mean(abs(np.array(R1_cal) - R_sim))
mean_R2_err = np.mean(abs(np.array(R2_cal) - R_sim))

print(mean_R0_err, mean_R1_err, mean_R2_err)
