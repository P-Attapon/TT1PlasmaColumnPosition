...
def compute_weights(shot_path, probes, discharge_current=2500,
    ...
    shot_path         : directory with IP1/IT1/IT2/IOH1/IOH2/IV1/IV2/GBP*T .txt
                        files. *2 variants are used by the current_channels resolver
                        (tolerated if absent).
    ...
    t, Ip = load("IP1")
    # Resolve redundant current channels; raises if both dead, falls back to
    # the live one with a log message if one is dead, averages if both healthy.
    from .current_channels import resolve_all as _resolve_all
    _ch, _ = _resolve_all(shot_path, discharge_current)
    It  = _ch["IT"][:len(t)]
    Ioh = _ch["IOH"][:len(t)]
    Iv  = _ch["IV"][:len(t)]
