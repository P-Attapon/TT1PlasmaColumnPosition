...
def mk_nested_dict_coeff(shift_domain, taylor_order=3, decimal_precision = base_decimal_precision):
    ...
    # Provenance sidecar. This .pkl is the ONE cached artefact with a FIXED
    # filename, so the name can encode nothing about what produced it, and
    # plasma_shift.py loads it unconditionally at import time. Written as a
    # SIDECAR rather than a third pickle.dump so the .pkl format stays exactly
    # as the original repo wrote it -- anything that reads it with two loads
    # keeps working unchanged.
    meta = {"fm_key": forward_model_key(), "model": describe(),
