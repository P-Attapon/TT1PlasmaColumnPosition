...
# ---- provenance check on the coefficient cache -----------------------------
# The Taylor coefficients are swept over [-shift_domain, shift_domain] and built
# from cal_signal, so they are a function of the forward model. Unlike the Phi
# maps this file has a FIXED name, so the name cannot record what produced it --
# hence the sidecar written by coefficient.py.
#
# This is a WARNING, not a refusal, and the asymmetry with mprobe.py is
# deliberate: this module is imported at package-import time by anything that
# touches TFM, including the M-probe path that never reads these coefficients at
# all. Raising here would take down runs that do not depend on the stale artefact.
# The 1D coefficient path itself is legacy (superseded by phi_map's 2D inverse),
# so a loud warning is proportionate to how it is actually used.
_meta_path = Path(str(pkl_path) + ".meta.json")
try:
    from .cache_keys import forward_model_key as _fm_key, describe as _fm_desc
    if not _meta_path.exists():
        print(f"[plasma_shift] WARNING: {pkl_path.name} has no provenance "
              f"sidecar (built before cache stamping). It cannot be verified "
              f"against the current forward model ({_fm_desc()}). If the 1D "
              f"coefficient path matters to you, rebuild it by running "
              f"coefficient.py; run  python stamp_model_caches.py --apply  to "
              f"assert it is already current.")
    else:
        _m = json.loads(_meta_path.read_text())
        if _m.get("fm_key") != _fm_key():
            print(f"[plasma_shift] WARNING: {pkl_path.name} was built under a "
                  f"DIFFERENT forward model (stamp {_m.get('fm_key')} != current "
                  f"{_fm_key()}; now {_fm_desc()}). Taylor coefficients from it "
                  f"are STALE -- rebuild by running coefficient.py. The 2D Phi "
                  f"path (phi_map.py) is unaffected.")
except Exception as _e:   # never let a provenance CHECK break an import
    print(f"[plasma_shift] provenance check skipped ({type(_e).__name__}: {_e})")
# ----------------------------------------------------------------------------
...
def toroidal_filament_shift_progression(time_df:pd.DataFrame,signal_df:pd.DataFrame,probe_number:list[list[int]],taylor_order:int = 3,DxDz_method = cal_newton_DxDz):
    ...
    for t, signal in tqdm(zip(
        ...
        for i,s in enumerate(signal_df):
            ...
            # the 2D map (current Dx,Dz only); these previous-step values do not
