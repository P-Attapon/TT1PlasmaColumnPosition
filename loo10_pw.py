...
DROP=10; flat=np.where((t[:m]>t0+2)&(t[:m]<t1-2))[0][::10]
...
for pw in (2.0, 1.5):
    ...
    for k in flat:
        ...
        if all(np.isfinite(x) for x in (r1,z1,r2,z2)):
            moves.append(np.hypot(r2-r1,z2-z1)*1e3); df.append((r1,z1)); dd.append((r2,z2))
    moves=np.array(moves); df=np.array(df)*1e3; dd=np.array(dd)*1e3
    print("power %.1f | GBP10 weight %.1f%% | drop-move median %.2f p90 %.2f max %.2f mm | Z: full %.1f -> minus10 %.1f mm | cond %.2f->%.2f"%(
        pw, 100*w[DROP]/tot, np.median(moves), np.percentile(moves,90), moves.max(),
        df[:,1].mean(), dd[:,1].mean(), full.cond, sub.cond))
